...
<body>
	...
	<div class="cs_center">
		...
		<form name="formUpdate" action="noticeUpdate.see" method="post">
			...
			<div>
				제목 <input name="notice_subject" id="notice_subject"
					value="${noticeModel.notice_subject}" size="80"
					placeholder="제목을 입력해주세요." />
			</div>
			<div>
				내용
				<textarea name="notice_content" id="notice_content" rows="4"
					cols="80" placeholder="내용을 입력해주세요.">${noticeModel.notice_content}</textarea>
			</div>
			<div>
				영화관 <input name="notice_area" id="notice_area"
					value="${noticeModel.notice_area}" placeholder="영화관을 입력해주세요." />
			</div>
