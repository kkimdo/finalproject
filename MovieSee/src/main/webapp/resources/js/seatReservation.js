    ...
    $.fn.seatReservation.prototype = {
        ...
        init: function () {
           // $('.footBnImg').hide();
            ...
            // 좌석 데이터
            var obj = { MethodName: "GetSeats", channelType: "HO", osType: BrowserAgent(), osVersion: navigator.userAgent, cinemaId: this.cinemaCode, screenId: this.screenCode, playDate: this.playDate, playSequence: this.playSequence, screenDivisionCode: this.screenDiv }; // 좌석정보
        ...
        seatDraw: function () {
            ...
            seatNode.each(function (n) {
            	row_num = seatNode[n].SeatRow;
            	group_num = seatNode[n].SeatColumGroupNo;

            	if (row_old != row_num || group_old != group_num) {
            		group_idx++;
            	}
            	row_old = row_num;
            	group_old = group_num;

            	seatNode[n].SeatColumGroupNo = group_idx;
            });
            ...
            seatNode.each(function (n) {
                ...
                var seatStatusCode = $(this)[0].SeatStatusCode; // 170831 cok - 시네패밀리 관 분리 - 50 일 경우 고장 좌석으로 표시
            	...
            	// 170831 cok - 시네패밀리 관 분리 - 월드타워9관 50 일 경우 고장 좌석으로 표시
                if (T.screenCode == "101609" && T.screenDiv == "960" && seatStatusCode == "50") {
                	searClass = 'disabled';
                	seatAlt = '선택불가';
                }
                ...
                if (saveRow != seatRow) {                                                                           // 열이 바뀌는 경우
                	...
                	// 170831 cok - 시네패밀리 관 분리 - seatStatusCode 추가 (좌석 상태)
                    ...
                    html += '<a href="javascript:void(0);" class="' + searClass + ' p' + feeCell + '" data-seat="' + seatNum + '" seat-statuscode="' + seatStatusCode + '" block-code="p'
			...
			// 170831 cok - 시네패밀리 관 분리 - 월드타워9관 시네패밀리
            if (T.screenCode == "101609" && T.screenDiv == "960") {
            	T.seatScroll.find('.btn_sView').addClass('on').next('.seat_Barea').show();
            	T.seatScroll.find('.bg_disabled').css('height', T.seatScroll.height()).show();
            } else {
        ...
        selected: function () {
            ...
            //20171030 권재익 책임 요청
            //brandName 이 있으면 brandName 으로 
            if (typeof T.brandName != "undefined" && T.brandName.length > 0) {
                var brandName = T.brandName.toString();             // 20171030  
                if (brandName != "") {
        ...
        event: function () {
        	...
        	// 170831 cok - 시네패밀리 관 분리 - 월드타워9관 시네패밀리 // this.cinemaCode:1016, this.screenCode:101609
            if (T.screenCode != "101609") {
            ...
            T.personSelect.find('.select_box').find('select').change(function () {
                ...
                */
            	...
            	// 170831 cok - 시네패밀리 관 분리 - 월드타워9관 시네패밀리 좌석상태가 50이 아닌것만  = 고장좌석이 아닌것만(시네패밀리 관 분리 작업)
                if (T.screenCode == "101609" && T.screenDiv == "960") {
                	T.seatArea.find('a').not("[seat-statuscode=50]").removeClass('disabled');
                } else {
